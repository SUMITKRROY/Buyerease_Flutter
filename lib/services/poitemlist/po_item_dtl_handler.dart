...
import '../ItemInspectionDetail/ItemInspectionDetailHandler.dart'; // Placeholder: Define this
...
developer.log("QRPOItemDtl_PKG_App_Details >>${jsonEncode(poItemDtl)}");
      final rows = await db.update(
  ...
  static Future<bool> insertPOItemDtlPKGAppDetails(
    ...
    try {
      final db = await DatabaseHelper().database;
      String date = ApiRoute.getCurrentDate();
      String? userid = await UserMasterTable().getFirstUserID();
      developer.log("context.select((SubjectBloc bloc) => bloc.state) $date");
      final contentValues = {
        'pRowID': poItemDtl.pRowID,
  ...
  static Future<bool> insertOnSite(
    ...
    try {
      ...
      // Step 1: Check if the OnSiteTestID already exists
      final check = await db.query(
        'QRPOItemDtl_OnSite_Test',
        where: 'OnSiteTestID = ?',
        whereArgs: [onSiteModal.onSiteTestID],
      );

      if (check.isNotEmpty) {
        print('$_tag: OnSiteTestID ${onSiteModal.onSiteTestID} already available.');
        return false; // Skip insert
      }

      // Step 2: Prepare values for insertion
      final contentValues = {
        'pRowID': onSiteModal.pRowID,
        'SampleSizeID': onSiteModal.sampleSizeID,
  ...
  static Future<bool> deleteSampleCollected(String samplePurposeID) async {
    try {
      final db = await DatabaseHelper().database;
      final rows = await db.delete(
        'QRPOItemDtl_Sample_Purpose',
        where: 'SamplePurposeID = ?',
        whereArgs: [samplePurposeID],
      );
  ...
  static Future<bool> insertSampleCollected(
      SampleCollectedModal sampleCollectedModal, POItemDtl poItemDtl) async {
    try {
      final db = await DatabaseHelper().database;
      String? userId = await UserMasterTable().getFirstUserID();

      // Step 1: Check for existing record with same SamplePurposeID and QRPOItemDtlID
      final existing = await db.query(
        'QRPOItemDtl_Sample_Purpose',
        where: 'SamplePurposeID = ? AND QRPOItemDtlID = ?',
        whereArgs: [sampleCollectedModal.samplePurposeID, poItemDtl.pRowID],
      );

      if (existing.isNotEmpty) {
        print('$_tag: SamplePurposeID already exists in the database for this item.');
      ...
      final contentValues = {
        ...
        'QRPOItemDtlID': poItemDtl.pRowID,
        'recEnable': 1,
        'recAddDt': ApiRoute.getCurrentDate(),
        'recUser': userId,
      };

      final result = await db.insert('QRPOItemDtl_Sample_Purpose', contentValues);
  ...
  static Future<List<SampleCollectedModal>> getSampleCollectedList() async {
    final List<SampleCollectedModal> sampleList = [];
    try {
      final db = await DatabaseHelper().database;
  ...
  static SampleCollectedModal _getSampleData(Map<String, dynamic> row) {
    return SampleCollectedModal(
      pRowID: row['pRowID']?.toString(),
      locID: row['locID']?.toString(),
      samplePurposeID: row['SamplePurposeID']?.toString(),
      sampleNumber: row['SampleNumber'] as int?,
      recDt: row['recDt']?.toString(),
    );
  }
  ...
  static Future<List<OnSiteModal>> getOnSiteList() async {
    ...
    try {
      ...
      final result = await db.rawQuery(query);
developer.log("result of data ${result}");
      for (var row in result) {
  ...
  static OnSiteModal _getOnSiteData(Map<String, dynamic> row) {
    return OnSiteModal(
      pRowID: row['pRowID']?.toString(),
      locID: row['LocID']?.toString(),
      inspectionLevelID: row['InspectionLevelID']?.toString(),
      inspectionResultID: row['InspectionResultID']?.toString(),
      onSiteTestID: row['OnSiteTestID']?.toString(),
      sampleSizeID: row['SampleSizeID']?.toString(),
      sampleSizeValue: row['SampleSizeValue']?.toString(),
      recEnable: row['recEnable'] as int?,
      recUser: row['recUser']?.toString(),
      recDt: row['recDt']?.toString(),
    );
  }
  ...
  static Future<List<DefectMasterModel>> getDefectMasterList() async {
      ...
      try {
          ...
          developer.log('$_tag: Count of founded defect list: ${jsonEncode(defectList)}');
      } catch (e) {
  ...
  static DefectMasterModel _getDefectMasterData(Map<String, dynamic> row) {
      return DefectMasterModel(
          pRowID: row['pRowID']?.toString(),
          defectName: row['DefectName']?.toString(),
          code: row['Code']?.toString(),
          dclTypeID: row['DCLTypeID']?.toString(),
          recAddUser: row['recAddUser']?.toString(),
          inspectionStage: row['InspectionStage']?.toString(),
  ...
  static Future<bool> updatePOItemHdrOnInspection(POItemDtl poItemDtl) async {
    try {
      ...
      print('$_tag: QRPOItemHdr table NOT UPDATED...$contentValues');
      if (rows == 0) {
        print('$_tag: QRPOItemHdr table NOT UPDATED...');
      } else {
        print('$_tag: QRPOItemHdr type update result: $rows');
  ...
  static Future<List<POItemDtl>> getItemList(String pInspectionID) async {
    ...
    try {
      ...
      for (var row in result) {
        if (poItemDtl.qrItemID == row['pRowID']) {
          latestDelDate = row['LatestDelDt']?.toString() ?? '';
        }
      }
  ...
  static Future<List<String>> getItemIdList(String pInspectionID) async {
    ...
    try {
      ...
      for (var row in result) {
        itemIdList.add(row['CustomerItemRef']?.toString() ?? '');
      }
  ...
  static Future<List<POItemPkgAppDetail>> getPKGAPPList() async {
    ...
    try {
      ...
      print('$_tag: Query for get PO from QRPOItemdtl list: $query');
      final result = await db.rawQuery(query);
      print('$_tag: Query for get PO from QRPOItemdtl list: $result');
      for (var row in result) {
        pkgAppList.add(POItemPkgAppDetail(
          pRowID: row['pRowID']?.toString(),
          locID: row['LocID']?.toString(),
          descrID: row['DescrID']?.toString(),
          sampleSizeID: row['SampleSizeID']?.toString(),
          sampleSizeValue: row['SampleSizeValue']?.toString(), // ✅ FIXED
          inspectionResultID: row['InspectionResultID']?.toString(),
          recUser: row['recUser']?.toString(),
        ));
      }
      ...
      String pRowInspection, int availableQty) async
  {
    try {
      ...
      final result = await db.rawQuery(query, [pRowInspection, availableQty, pRowInspection]);
developer.log("message result $result");
      if (result.isNotEmpty) {
        final row = result.first;
        final toInspArray = [
          row['SampleSizeID']?.toString() ?? '',
          row['SampleSizeDescr']?.toString() ?? '',
          row['SampleSizeQty']?.toString() ?? '',
        ];
        print(
  ...
  static Future<List<SampleModel>> getSampleSizeList() async {
    ...
    try {
      ...
      final result = await db.rawQuery(query);
      print('$_tag: Query for get sample size: $result');

      for (var row in result) {
        sampleList.add(SampleModel(
          sampleCode: row['SampleCode']?.toString(),
          mainDescr: row['MainDescr']?.toString(),
          sampleVal: (row['numVal1'] != null)
              ? (row['numVal1'] as num).toInt()
              : null,
        ));
      }
